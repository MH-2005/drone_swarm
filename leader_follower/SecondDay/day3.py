...
def main():
    ...
    p.add_argument("--mav", type=str, default="udpin://:14540", help="MAVSDK connection string")
    p.add_argument("--listen-port", type=int, default=DEFAULT_LISTEN_PORT, help="UDP listen/broadcast port")
    p.add_argument("--bcast-ip", type=str, default=DEFAULT_BCAST_IP, help="broadcast ip")
    args = p.parse_args()

    node = FollowerRect(node_id=args.node_id, mav_conn=args.mav, listen_port=args.listen_port, bcast_ip=args.bcast_ip)
    try:
